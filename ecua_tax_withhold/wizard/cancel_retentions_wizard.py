...
class cancel_retention_wizard(osv.osv_memory):
    ...
    _name = 'account.cancel.retention.wizard' 
    
    _columns = {
                    'line_ids':fields.one2many('account.cancel.retention.line.wizard', 'wizard_id', 'Lines', required=True), 
    ...
    def action_cancel_retentions(self, cr, uid, ids, context=None):
        ...
        wiz_l_obj = self.pool.get('account.cancel.retention.line.wizard')
...
class cancel_retention_line_wizard(osv.osv_memory):
    ...
    _name = 'account.cancel.retention.line.wizard'
    
    _columns = {
                    'wizard_id':fields.many2one('account.cancel.retention.wizard', 'Wizard', required=False),
